...
<body class="easyui-layout" style="padding: 2px;">
	<table id="dg" class="easyui-datagrid" style="height:100%;display: none" 
		data-options="singleSelect:false,pageSize:50,
		...
		collapsible="true"				
		url="task/loadCompleteAndNeedMyCloseTask"
		rownumbers="true"
		title="任务关闭" toolbar="#tb" pagination="true">
	...
	<thead>
		<tr>
			...
			<th field="TASK_STATE" align="center" width="60px" >任务状态</th>	
			
		</tr>
	</thead>
</table>
<div id="tb" style="display:none">
	<a href="javascript:;" id="okBtn"class="easyui-linkbutton" iconCls="icon-ok" plain="true">关闭</a>
	<a href="javascript:;" id="okAllBtn"class="easyui-linkbutton" iconCls="icon-ok" plain="true">关闭所有</a>
	<a href="javascript:;" id="redoBtn"class="easyui-linkbutton" iconCls="icon-redo" plain="true">退回重做</a>
	...
	$(function(){
	   ...
	   $("#addBtn").on("click",function(){
		...
		$("#okBtn").on("click",function(){
			...
			$.messager.confirm('', '确认要关闭选择的任务？', function(r){
				if (r){
					$.messager.progress({title:"",msg:"正在处理,请稍后",text:"",interval:500});
					//提交服务器删除
					$.ajax({
						type: "POST",
						url: "task/task_closeTask",
						dataType:'json',
						data:{"PTIDS":PTIDS.join(",")},
						cache: false,
		...
		//关闭所有
		$("#okAllBtn").on("click",function(){			
			$.messager.confirm('', '确认要关闭所有任务？', function(r){
				if (r){
					$.messager.progress({title:"",msg:"正在处理,请稍后",text:"",interval:500});
					//提交服务器删除
					$.ajax({
						type: "POST",
						url: "task/task_closeAllTask",
						dataType:'json',						
						cache: false,
						success: function(data){
							$.messager.progress('close');
